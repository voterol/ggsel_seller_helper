...
class TelegramBot:
    def __init__(self, config: Config):
        ...
        self.sync_nomessage_handler = None
    ...
    def set_sync_nomessage_handler(self, h): self.sync_nomessage_handler = h
    ...
    async def start(self):
        ...
        try:
            ...
            self.application.add_handler(CommandHandler("sync_nomessage", self._handle_sync_nomessage_command))
    ...
    async def _handle_sync_nomessage_command(self, update: Update, context):
        if not self._is_authorized(update) or not self.sync_nomessage_handler:
            return
        text = await self.sync_nomessage_handler()
        await update.effective_message.reply_text(text)
